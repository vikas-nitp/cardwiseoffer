    ...
    <>
      <header className="w-full py-4 px-4 md:px-8 flex items-center justify-between relative z-20">
        <Link to="/" className="flex items-center gap-2.5 group" onClick={() => handleNav("home")}>
          <div className="w-9 h-9 rounded-xl bg-primary flex items-center justify-center shadow-sm"><Plane className="w-4.5 h-4.5 text-primary-foreground" /></div>
          <span className="text-lg font-bold text-foreground tracking-tight">CardWiseOffer</span>
        </Link>
        <nav className="hidden md:flex items-center gap-0.5">
          {navItems.map(({ label, section }) => <button key={section} onClick={() => handleNav(section)} className={cn("px-3.5 py-2 rounded-lg text-[13px] font-medium", activeSection === section ? "bg-primary/8 text-primary font-semibold" : "text-muted-foreground hover:text-foreground hover:bg-muted/50")}>{label}</button>)}
        </nav>
      ...
      {mobileOpen && <div className="fixed inset-0 z-30 md:hidden">
        ...
        <nav className="absolute top-0 right-0 w-72 h-full bg-card shadow-2xl p-6 pt-16 flex flex-col gap-1">
          ...
          {navItems.map(({ label, section }) => <button key={section} onClick={() => handleNav(section)} className={cn("w-full text-left px-4 py-3 rounded-xl text-sm font-medium", activeSection === section ? "bg-primary/8 text-primary font-semibold" : "text-muted-foreground hover:text-foreground hover:bg-muted/50")}>{label}</button>)}
        </nav>
      </div>}
    </>
  );
